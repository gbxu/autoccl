...
namespace {
  ...
  __device__ __forceinline__ void runRing(ncclWorkElem *args) {
    ...
    ssize_t chunkSize;
    if (args->native) {
      chunkSize = int(Proto::calcBytePerStep()/sizeof(T) * (Proto::Id == NCCL_PROTO_SIMPLE ? ALLREDUCE_CHUNKSTEPS : 1));
    } else {
    ...
    if (tid == 0) {
      NpKit::CollectGpuEvent(NPKIT_EVENT_TIME_SYNC_GPU, 0, ringIx, clock64(),
          ncclShmem.comm.npKitEventCollectContexts + npKitCtxIdx);
    }
#endif
    ...
    Primitives<T, RedOp, FanSymmetric<1>, 1, Proto, 0> prims
      (tid, nthreads, &ring->prev, &ring->next, args->sendbuff, args->recvbuff, args->redOpArg, 0, 0, 0, args->transportIndex);
    ...
    if (tid == 0) {
      NpKit::CollectGpuEvent(NPKIT_PRIM_COLLECT_DATA_PROCESS_TIME, size*sizeof(T), 0, prims.npKitDataProcessTotalTime,
          ncclShmem.comm.npKitEventCollectContexts + npKitCtxIdx);
    }
#endif
  ...
  __device__ __forceinline__ void runTreeUpDown(ncclWorkElem *args) {
    ...
    { // Reduce : max number of recv is 3, max number of send is 1 (binary tree + local)
      Primitives<T, RedOp, FanAsymmetric<NCCL_MAX_TREE_ARITY, 1>, /*Direct=*/0, Proto, 0> prims
        (tid, nthreads, tree->down, &tree->up, args->sendbuff, args->recvbuff, args->redOpArg, 0, 0, 0, args->transportIndex);
    ...
    { // Broadcast : max number of recv is 1, max number of send is 3 (binary tree + local)
      Primitives<T, RedOp, FanAsymmetric<1, NCCL_MAX_TREE_ARITY>, /*Direct=*/1, Proto, 0> prims
        (tid, nthreads, &tree->up, tree->down, args->sendbuff, args->recvbuff, args->redOpArg, 0, 0, 0, args->transportIndex+NCCL_MAX_TREE_ARITY+1);
  ...
  __device__ __forceinline__ void runTreeSplit(ncclWorkElem *args) {
    ...
    int nthreadsSplit;
    if (Proto::Id == NCCL_PROTO_SIMPLE) {
      nthreadsSplit = (nthreads/(2*WARP_SIZE))*WARP_SIZE;
    ...
    if (tree->up == -1) {
      ...
      Primitives<T, RedOp, FanSymmetric<NCCL_MAX_TREE_ARITY_TOP>, /*Direct=*/1, Proto, 0>
        prims(tid, nthreads, tree->down, tree->down, args->sendbuff, args->recvbuff, args->redOpArg, 0, 0, 0, args->transportIndex);
      ...
      Primitives<T, RedOp, FanAsymmetric<NCCL_MAX_TREE_ARITY, 1>, /*Direct=*/1, Proto, 0>
        prims(tid, nthreadsSplit, tree->down, &tree->up, args->sendbuff, args->recvbuff, args->redOpArg, 0*Proto::MaxGroupWidth, 0, 0, args->transportIndex);
      ...
      Primitives<T, RedOp, FanAsymmetric<1, NCCL_MAX_TREE_ARITY>, /*Direct=*/1, Proto, 0>
        prims(tid-nthreadsSplit, nthreads-nthreadsSplit, &tree->up, tree->down, args->sendbuff, args->recvbuff,
            args->redOpArg, 1*Proto::MaxGroupWidth, 0, 0, args->transportIndex+NCCL_MAX_TREE_ARITY+1);
  ...
  __device__ __forceinline__ void run(ncclWorkElem *args) {
    if (args->native) {
      ...
      switch (args->userSliceSteps) {
        case 1:
          runRing<T, RedOp, ProtoSimple<ALLREDUCE_CHUNKSTEPS/1/*slicePerChunk=4*/, 1/*StepPerSlice=1*/>>(args);
          break;
        case 2:
          runRing<T, RedOp, ProtoSimple<ALLREDUCE_CHUNKSTEPS/2/*slicePerChunk=2*/, 2/*StepPerSlice=2*/>>(args);
          break;
        case 4:
          runRing<T, RedOp, ProtoSimple<ALLREDUCE_CHUNKSTEPS/4/*slicePerChunk=1*/, 4/*StepPerSlice=4*/>>(args);
          break;
      }
    }
  }
...
struct RunWorkElement<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_TREE, NCCL_PROTO_SIMPLE> {
  __device__ __forceinline__ void run(ncclWorkElem *args) {
    ...
    // #endif
    if (args->useAllReduceSimpleTreeUpDown) {
      runTreeUpDown<T, RedOp, ProtoSimple<1, 1>>(args);
    } else {
      runTreeSplit<T, RedOp, ProtoSimple<1, 1>>(args);
    }
    ...
    if (tid >= tidStartScatter && tid < tidStartReduce && hasUp) {
      ...
      Primitives<T, RedOp, FanAsymmetric<0, NCCL_MAX_DIRECT_ARITY>, /*Direct=*/1, Proto, 0>
        prims(tid-tidStartScatter, nThreadsScatter, NULL, direct->up, args->sendbuff, args->recvbuff,
           args->redOpArg, 2*Proto::MaxGroupWidth, 1, 1, nullptr, args);
      for (ssize_t gridOffset = 0; gridOffset < size; gridOffset += loopSize) {
      ...
      if (hasDn) {
        ...
        Primitives<T, RedOp, FanAsymmetric<NCCL_MAX_DIRECT_ARITY, 1>, /*Direct=*/1, Proto, 0>
          prims(tid-tidStartReduce, nThreadsReduce, direct->down, &direct->out, args->sendbuff, args->recvbuff,
             args->redOpArg, 3*Proto::MaxGroupWidth, 1, 1, nullptr, args);
        for (ssize_t gridOffset = 0; gridOffset < size; gridOffset += loopSize) {
      ...
      Primitives<T, RedOp, FanAsymmetric<NCCL_MAX_DIRECT_ARITY, 0>, /*Direct=*/1, Proto, 0>
        prims(tid, nThreadsGather, direct->up, NULL, args->sendbuff, args->recvbuff,
           args->redOpArg, 0*Proto::MaxGroupWidth, 0, 0, nullptr, args);
      for (ssize_t gridOffset = 0; gridOffset < size; gridOffset += loopSize) {
        ssize_t offset = gridOffset + bid*direct->nHeads*chunkSize;
        int nelem = min(direct->nHeads*chunkSize, size-offset);
      ...
      if (hasDn) {
        ...
        Primitives<T, RedOp, FanAsymmetric<1, NCCL_MAX_DIRECT_ARITY>, /*Direct=*/1, Proto, 0>
          prims(tid-tidStartBcast, nThreadsBcast, &direct->out, direct->down, args->sendbuff, args->recvbuff,
             args->redOpArg, 1*Proto::MaxGroupWidth, 0, 0, nullptr, args);
        for (ssize_t gridOffset = 0; gridOffset < size; gridOffset += loopSize) {
          ssize_t offset = gridOffset + (bid*direct->nHeads+direct->headRank)*chunkSize;
          int nelem = min(chunkSize, size-offset);
...
struct RunWorkElement<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_TREE, NCCL_PROTO_LL> {
  __device__ __forceinline__ void run(ncclWorkElem *args) {
    if (args->useAllReduceLL128LLTreeUpDown) {
...
struct RunWorkElement<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_TREE, NCCL_PROTO_LL128> {
  __device__ __forceinline__ void run(ncclWorkElem *args) {
    if (args->useAllReduceLL128LLTreeUpDown) {
      runTreeUpDown<T, RedOp, ProtoLL128>(args);
    } else {
