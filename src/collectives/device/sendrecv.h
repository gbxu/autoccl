...
struct RunWork<ncclFuncSendRecv, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE> {
  ...
  __device__ void runSend(const int tid, const int nthreads, const uint8_t group, struct ncclWorkElemP2p* args) {
    ...
    if (args->peer == ncclShmem.comm.rank) {
      ...
      // let args->chunkSize effective chunksize
      int chunkSize = args->effectiveChunkSize/sizeof(T);
      // if (args->proto == NCCL_PROTO_LL) chunkSize /= 2;
      int const peer = args->peer;
      uint8_t transportIndex = args->transportIndex;
      Primitives<T, RedOp, FanAsymmetric<0, 1>, 1, Proto, 1> prims
        (tid, nthreads, nullptr, &peer, buff, nullptr, /*redOpArg(ignored)=*/0, group, 1, 1, &transportIndex);
  ...
  __device__ void runRecv(const int tid, const int nthreads, const uint8_t group, struct ncclWorkElemP2p* args) {
    ...
    if (args->peer != ncclShmem.comm.rank) {
      ...
      Primitives<T, RedOp, FanAsymmetric<1, 0>, 1, Proto, 1> prims
        (tid, nthreads, &peer, nullptr, nullptr, buff, /*redOpArg(ignored)=*/0, group, 1, 1, &transportIndex);
  ...
  __device__ __forceinline__ void run(ncclWork *work) {
    ...
    uint8_t group;
    if (args->native) {
      group = ngroups-1- (NWARPS-1-wid) * ngroups / NWARPS;
      GPUPRINT(true, "group=%d", group);
    } else {
      group = wid / args->nWarps;
      GPUPRINT(true, "group=%d", group);
    }
    args += group;
    tid -= args->warpStart * WARP_SIZE;
    int nthreads = args->nWarps * WARP_SIZE;
    GPUPRINT(tid == 0, "group=%d/%d channel=%d peer=%d %s warpStart=%d, nWarps=%d", group, ngroups, ncclShmem.channelId, args->peer, (args->p2pType==ncclWorkP2pTypeSend? "send":"recv"), args->warpStart, args->nWarps);
    ...
    if ((group%2) == 0) {
      ...
      } else if (args->proto == NCCL_PROTO_LL128) {
        runRecv<ProtoLL128>(tid, nthreads, group, args);
      } else {
      ...
      if (args->proto == NCCL_PROTO_LL) {
        ...
        runSend<ProtoLL128>(tid, nthreads, group, args);
      } else {
