...
class NpKit {
 public:
  static const uint64_t kNumGpuEventBuffers = 64; // [32, 64) for TreeSlipt

  static const uint64_t kNumCpuEventBuffers = 64; // [32, 64) for TreeSlipt
 ...
 private:
  ...
  // max: 2M * 32 * 16B = 1GB per GPU
  static const uint64_t kMaxNumGpuEventsPerBuffer = 1ULL << 21; // 2M

  // max: 4M * 32 * 16B = 2GB per CPU
  static const uint64_t kMaxNumCpuEventsPerBuffer = 1ULL << 22; // 2M * 2 (send/recv) * (32/32) = 4M
